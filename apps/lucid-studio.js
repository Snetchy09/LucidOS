...
function openLucidScriptEditor(root, project = null) {
    ...
    root.querySelector("#studio-editor-manifest")?.addEventListener("click", () => { stopActiveRuntime(root); saveCurrentStudioProject(); showStudioManifest(root); });
...
const activeRuntimes = new WeakMap();
function stopActiveRuntime(root) { const previous = activeRuntimes.get(root); if (previous?.stop) { try { previous.stop(); } catch {} } activeRuntimes.delete(root); }
function runStudioCode(root) {
    stopActiveRuntime(root);
    ...
    preview.innerHTML = "";
    try { const result = runLucidScript(code, preview, { permissions: [] }); activeRuntimes.set(root, result); status.textContent = `Running: ${result.appName}`; }
