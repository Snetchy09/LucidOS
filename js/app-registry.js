...
import { recordAppDownload } from "./lucid-store-api.js";
...
function saveExternalApps(apps) { localStorage.setItem(EXTERNAL_APPS_KEY, JSON.stringify(apps)); }
function writeInstalledAppFile(app) {
    if (!app || !app.id || app.type === "core") return;
    const manifest = { type: "lucid-app", id: app.id, name: app.name, version: app.version || "1.0.0", category: app.category || "Other", description: app.description || "", package_key: app.package_key || null, source_key: app.source_key || null, entry_point: app.entry_point || null };
    saveUserFile(["Downloads"], `${app.id}.lucidapp`, JSON.stringify(manifest, null, 2), "application/json").catch(error => console.warn("Lucid: could not save app package:", error));
}
function removeInstalledAppFile(appId) { deleteUserFile(["Downloads"], `${appId}.lucidapp`).catch(() => {}); }
function installExternalApp(app) {
    ...
    const installed = { id: app.id, name: app.name, icon: app.icon || app.icon_url || "◇", version: app.version || "1.0.0", category: app.category || "Other", description: app.description || "", type: app.app_type || "community", package_key: app.package_key || null, source_key: app.source_key || null, entry_point: app.entry_point || null, installed: true, external: true };
    externalApps.push(installed);
    saveExternalApps(externalApps);
    writeInstalledAppFile(installed);
    recordAppDownload(app.id, app.version).catch(() => {});
...
function saveInstalledIds(ids) { localStorage.setItem(APP_REGISTRY_KEY, JSON.stringify([...new Set(ids)])); }
...
function getApp(appId) { return appCatalog.find(app => app.id === appId) || getExternalApps().find(app => app.id === appId) || null; }
function isAppInstalled(appId) { return getInstalledIds().includes(appId) || getExternalApps().some(app => app.id === appId); }
...
function getAppLauncher(appId) { return appLaunchers[appId] || null; }
...
export { appCatalog, initializeAppRegistry, getInstalledApps, getAllApps, getApp, isAppInstalled, installApp, installExternalApp, uninstallApp, registerApp, registerAppLauncher, getAppLauncher };